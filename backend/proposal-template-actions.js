...
function ensureSchema(){
  ...
  const startConfig=JSON.stringify({showStart:true,startPrice:9.99,contentCount:3,discountOnFirstMonth:10});
  ...
  db.prepare(`INSERT OR IGNORE INTO proposal_templates (key,name,description,type,is_active,is_default,config) VALUES ('duit-start','DUIT Start — 9,99 €','Planos mensais + experiência DUIT Start com 3 conteúdos.','duit_start',1,0,?)`).run(startConfig);
  db.prepare(`UPDATE proposal_templates SET name='DUIT Start — 9,99 €',config=? WHERE key='duit-start'`).run(startConfig);
