...
module.exports = function installMetaReports(app) {
  ...
  async function getPageCredential(pageId) {
    const data = await metaGet('me/accounts', { fields: 'id,name,access_token,instagram_business_account{id,username},connected_instagram_account{id,username}', limit: 200 });
    ...
    return { pageToken: page.access_token, pageName: page.name || null, instagram_account_id: ig?.id || null, instagram_username: ig?.username || null };
  }
  ...
  async function supportedInsights(objectId, metrics, token, extra={}) {
    const out={}, unsupported=[];
    for (const metric of metrics) {
      try {
        const d = await metaGet(`${objectId}/insights`, { metric, ...extra }, token);
        const row = (d.data || [])[0];
        if (row) out[metric] = insightValue(row);
      } catch (e) { unsupported.push({ metric, error:e.message }); }
    }
    return { values:out, unsupported };
  }
  ...
  app.get('/api/meta/collect-test/:userId/:year/:month', requireAdmin, async(req,res)=>{
    try {
      const userId=Number(req.params.userId), asset=clientAssetRow(userId); if(!asset)return res.status(404).json({error:'Cliente sem conta Meta associada.'});
      const {since,until}=period(req.params.year,req.params.month);
      const result={client:{id:userId,name:asset.client_company||asset.client_name},period:{year:Number(req.params.year),month:Number(req.params.month)},facebook:{page_id:asset.page_id,page_name:asset.page_name,profile:{},posts:[],totals:{}},instagram:{account_id:asset.instagram_account_id,username:asset.instagram_username,profile:{},media:[],totals:{}},warnings:[],metric_support:{facebook:[],instagram:[]}};
      let pageCredential=null;

      if(asset.page_id){
        try {
          pageCredential=await getPageCredential(asset.page_id);
          try { result.facebook.profile=await metaGet(asset.page_id,{fields:'id,name,fan_count,followers_count'},pageCredential.pageToken); } catch(_) {}
          const fb=await metaGet(`${asset.page_id}/posts`,{fields:'id,message,created_time,permalink_url,shares,reactions.limit(0).summary(true),comments.limit(0).summary(true),attachments{media_type,type,url}',since,until,limit:100},pageCredential.pageToken);
          for(const p of (fb.data||[])){
            const metricResult=await supportedInsights(p.id,['post_impressions_unique','post_impressions','post_engaged_users','post_clicks'],pageCredential.pageToken);
            const metrics=metricResult.values;
            Object.keys(metrics).forEach(m=>{if(!result.metric_support.facebook.includes(m))result.metric_support.facebook.push(m);});
            result.facebook.posts.push({...p,metrics,reactions_count:p.reactions?.summary?.total_count||0,comments_count:p.comments?.summary?.total_count||0,shares_count:p.shares?.count||0});
          }
      ...
      if(igId){
        try {
          const token=pageCredential?.pageToken||centralToken();
          try { result.instagram.profile=await metaGet(igId,{fields:'id,username,name,followers_count,media_count'},token); } catch(_) {}
          const ig=await metaGet(`${igId}/media`,{fields:'id,caption,media_type,media_product_type,timestamp,permalink,like_count,comments_count',since,until,limit:100},token);
          for(const m of (ig.data||[])){
            const metricResult=await supportedInsights(m.id,['reach','impressions','saved','shares','total_interactions','views','plays'],token);
            const metrics=metricResult.values;
            Object.keys(metrics).forEach(k=>{if(!result.metric_support.instagram.includes(k))result.metric_support.instagram.push(k);});
            result.instagram.media.push({...m,metrics});
          }
        } catch(e){result.warnings.push(`Instagram: ${e.message}`);}
      }

      const sum=(arr,fn)=>arr.reduce((a,x)=>a+(Number(fn(x))||0),0);
      result.facebook.totals={posts:result.facebook.posts.length,reactions:sum(result.facebook.posts,p=>p.reactions_count),comments:sum(result.facebook.posts,p=>p.comments_count),shares:sum(result.facebook.posts,p=>p.shares_count),reach:sum(result.facebook.posts,p=>p.metrics?.post_impressions_unique),impressions:sum(result.facebook.posts,p=>p.metrics?.post_impressions),engaged_users:sum(result.facebook.posts,p=>p.metrics?.post_engaged_users),clicks:sum(result.facebook.posts,p=>p.metrics?.post_clicks)};
