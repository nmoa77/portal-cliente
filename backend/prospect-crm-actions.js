...
module.exports = function installProspectCrmActions(app) {
  ...
  add('email_tracking_token', 'TEXT'); add('email_sent_at', 'TEXT'); add('email_first_sent_at', 'TEXT'); add('email_first_opened_at', 'TEXT');
  // Prospects anteriores a esta coluna: a data que já existia era o primeiro envio.
  db.prepare(`UPDATE prospect_crm SET email_first_sent_at=email_sent_at WHERE email_first_sent_at IS NULL AND email_sent_at IS NOT NULL`).run(); add('email_last_opened_at', 'TEXT'); add('email_open_count', 'INTEGER DEFAULT 0'); add('proposal_first_viewed_at', 'TEXT'); add('proposal_last_viewed_at', 'TEXT'); add('proposal_view_count', 'INTEGER DEFAULT 0'); add('guide_first_opened_at', 'TEXT'); add('guide_last_opened_at', 'TEXT'); add('guide_open_count', 'INTEGER DEFAULT 0'); add('outreach_response', 'TEXT'); add('outreach_response_reason', 'TEXT'); add('outreach_responded_at', 'TEXT'); add('ebook_page_id', 'INTEGER'); add('outreach_question', 'TEXT'); add('outreach_question_at', 'TEXT');
