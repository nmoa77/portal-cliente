...
try {
  ...
  serverSource = serverSource.replace(/prospects-crm\.js\?v=[^'\"]+/g, 'prospects-crm.js?v=20260904i');
  ...
  if (!source.includes(marker)) source += `\n;(() => { if (document.querySelector('script[data-duit-prospect-actions]')) return; const s=document.createElement('script'); s.src='/js/prospects-actions.js?v=20260904i'; s.dataset.duitProspectActions='1'; document.body.appendChild(s); })();\n`;
  else source=source.replace(/prospects-actions\.js\?v=[^'\"]+/g,'prospects-actions.js?v=20260904i');
  ...
  // Filtros úteis no dia a dia: falta contactar, contactados, não abriram e não responderam.
  source = source.replace(/function filteredProspects\(\)\{[^}]*const q=crmFilter\.q\.trim\(\)\.toLowerCase\(\);[\s\S]*?\}\n  function renderCrmTable/, `function filteredProspects(){const q=crmFilter.q.trim().toLowerCase();return crmProspects.filter(p=>{if(crmFilter.status!=='all'){if(crmFilter.status==='falta_contactar'){if(p.email_sent_at)return false;}else if(crmFilter.status==='contactado'){if(!p.email_sent_at)return false;}else if(crmFilter.status==='nao_abriu'){if(!(p.email_sent_at&&!p.email_first_opened_at))return false;}else if(crmFilter.status==='nao_respondeu'){if(!(p.email_sent_at&&!p.outreach_response))return false;}}if(crmFilter.priority!=='all'&&p.priority!==crmFilter.priority)return false;if(!q)return true;return[p.company,p.name,p.email,p.phone,p.sector,p.location,p.opportunity,p.idea,p.notes].filter(Boolean).join(' ').toLowerCase().includes(q);})}\n  function renderCrmTable`);
  source = source.replace(/<select onchange="crmSetFilter\('status',this\.value\)"><option value="all">Todos os estados<\/option>\$\{Object\.entries\(STATUS\)\.map\(\(\[k,v\]\)=>`<option value="\$\{k\}">\$\{v\[0\]\}<\/option>`\)\.join\(''\)\}<\/select>/g, `<select onchange="crmSetFilter('status',this.value)"><option value="all">Todos</option><option value="falta_contactar">Falta contactar</option><option value="contactado">Contactados</option><option value="nao_abriu">Não abriu</option><option value="nao_respondeu">Não respondeu</option></select>`);
  // fallback para fontes já transformadas em deploys anteriores
  source = source.replace(/<select onchange="crmSetFilter\('status',this\.value\)"><option value="all">Todos os estados<\/option>[\s\S]*?<\/select><\/div><div class="card table-card"/, `<select onchange="crmSetFilter('status',this.value)"><option value="all">Todos</option><option value="falta_contactar">Falta contactar</option><option value="contactado">Contactados</option><option value="nao_abriu">Não abriu</option><option value="nao_respondeu">Não respondeu</option></select></div><div class="card table-card"`);
