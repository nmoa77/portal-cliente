...
try {
  ...
  } else source = source.replace(/prospects-actions\.js\?v=[^'\"]+/g, 'prospects-actions.js?v=20260827a');

  if (!source.includes('DUIT_AUTO_VERSION_REFRESH')) {
    source += `\n;(() => {\n  /* DUIT_AUTO_VERSION_REFRESH */\n  let knownVersion = null; let reloading = false;\n  async function checkVersion(){\n    if (reloading) return;\n    try {\n      const r = await fetch('/api/app-version?t=' + Date.now(), { cache:'no-store' });\n      if (!r.ok) return; const data = await r.json(); if (!data?.version) return;\n      if (knownVersion === null) { knownVersion = data.version; return; }\n      if (data.version !== knownVersion) { reloading = true; location.reload(); }\n    } catch (_) {}\n  }\n  checkVersion(); setInterval(checkVersion, 10000); window.addEventListener('focus', checkVersion);\n  document.addEventListener('visibilitychange', () => { if (!document.hidden) checkVersion(); });\n})();\n`;
  }
  ...
  const newLoader = `async function loadProspects(){
    ...
    crmProspects = (prospects||[]).map(p=>({...p,...(sm.get(Number(p.id))||{})})).sort((a,b)=>{
      ...
      if (ad !== bd) return bd - ad; return Number(b.id||0) - Number(a.id||0);
    }); return crmProspects;
  }`;
  ...
  const newFiltered = `function filteredProspects(){
    const q=crmFilter.q.trim().toLowerCase(); return crmProspects.filter(p=>{
      if(crmFilter.status!=='all'){
        if(crmFilter.status==='sem_resposta'){ if(!(p.email_sent_at && !p.outreach_response)) return false; }
        else if(crmFilter.status==='respondeu'){ if(!p.outreach_response) return false; }
        else if(p.lead_status!==crmFilter.status) return false;
      }
      if(crmFilter.priority!=='all'&&p.priority!==crmFilter.priority)return false;
      if(!q)return true; return [p.company,p.name,p.email,p.phone,p.sector,p.location,p.opportunity,p.idea].filter(Boolean).join(' ').toLowerCase().includes(q);
    });
  }`;
  ...
  source = source.replace('.crm-toolbar input,.crm-toolbar select{min-height:40px}.crm-toolbar .crm-search{flex:1;min-width:220px}', '.crm-toolbar input,.crm-toolbar select{min-height:40px}.crm-toolbar .crm-search{flex:1 1 320px;min-width:220px}.crm-toolbar select{width:auto;flex:0 0 auto;min-width:190px}@media(max-width:760px){.crm-toolbar .crm-search,.crm-toolbar select{width:100%;flex:1 1 100%;min-width:0}}');
  source = source.replace('<option value="all">Todas as prioridades</option><option value="atacar">🔥 Atacar</option><option value="possivel">🟡 Possível</option><option value="nao_prioritario">Não prioritário</option>', '<option value="all"${crmFilter.priority===\'all\'?\' selected\':\'\'}>Todas as prioridades</option><option value="atacar"${crmFilter.priority===\'atacar\'?\' selected\':\'\'}>🔥 Atacar</option><option value="possivel"${crmFilter.priority===\'possivel\'?\' selected\':\'\'}>🟡 Possível</option><option value="nao_prioritario"${crmFilter.priority===\'nao_prioritario\'?\' selected\':\'\'}>Não prioritário</option>');
  source = source.replace('<option value="all">Todos os estados</option>${Object.entries(STATUS).map(([k,v])=>`<option value="${k}">${v[0]}</option>`).join(\'\')}', '<option value="all"${crmFilter.status===\'all\'?\' selected\':\'\'}>Todos os estados</option>${Object.entries(STATUS).map(([k,v])=>`<option value="${k}"${crmFilter.status===k?\' selected\':\'\'}>${v[0]}</option>`).join(\'\')}');

  if (!source.includes('DUIT_CRM_NATIVE_PAGINATION')) {
    const paginationPatch = `\n  /* DUIT_CRM_NATIVE_PAGINATION */\n  let crmCurrentPage = 1; const CRM_PAGE_SIZE = 15; const crmRenderTableBase = renderCrmTable;\n  function crmApplyNativePagination(main) {\n    const table = main?.querySelector('.table-card table.table'); main?.querySelector('#crm-pagination-native')?.remove(); if (!table) return;\n    const rows = Array.from(table.querySelectorAll('tbody > tr')); const total = rows.length; const pages = Math.max(1, Math.ceil(total / CRM_PAGE_SIZE));\n    crmCurrentPage = Math.max(1, Math.min(crmCurrentPage, pages)); const start = (crmCurrentPage - 1) * CRM_PAGE_SIZE; const end = Math.min(start + CRM_PAGE_SIZE, total);\n    rows.forEach((row, index) => { row.style.display = index >= start && index < end ? '' : 'none'; });\n    const pager = document.createElement('div'); pager.id = 'crm-pagination-native'; pager.style.cssText = 'display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;padding:14px 2px 0;font-size:13px;color:var(--muted)';\n    if (total <= CRM_PAGE_SIZE) pager.innerHTML = '<span>' + total + ' prospect' + (total === 1 ? '' : 's') + '</span>';\n    else { const nums = Array.from({length:pages}, (_,i)=>i+1).map(n => '<button type="button" class="btn ' + (n===crmCurrentPage?'btn-yellow':'btn-ghost') + ' btn-sm" style="min-width:36px" onclick="crmGoPage(' + n + ')">' + n + '</button>').join(''); pager.innerHTML = '<span>A mostrar ' + (start + 1) + '–' + end + ' de ' + total + '</span><div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap"><button type="button" class="btn btn-ghost btn-sm" onclick="crmGoPage(' + (crmCurrentPage - 1) + ')" ' + (crmCurrentPage===1?'disabled':'') + '>‹ Anterior</button>' + nums + '<button type="button" class="btn btn-ghost btn-sm" onclick="crmGoPage(' + (crmCurrentPage + 1) + ')" ' + (crmCurrentPage===pages?'disabled':'') + '>Seguinte ›</button></div>'; }\n    table.closest('.table-card')?.insertAdjacentElement('afterend', pager);\n  }\n  renderCrmTable = function(main) { crmRenderTableBase(main); crmApplyNativePagination(main); };\n  window.crmGoPage = function(page) { const total = filteredProspects().length; const pages = Math.max(1, Math.ceil(total / CRM_PAGE_SIZE)); crmCurrentPage = Math.max(1, Math.min(Number(page) || 1, pages)); renderCrmTable(document.getElementById('main')); document.querySelector('#main .table-card')?.scrollIntoView({behavior:'smooth', block:'start'}); };\n  const crmSetFilterBase = window.crmSetFilter; if (typeof crmSetFilterBase === 'function') { window.crmSetFilter = function(key, value) { crmCurrentPage = 1; return crmSetFilterBase(key, value); }; }\n`;
...
} catch (e) { console.warn('[crm] não foi possível ligar ações/paginação de Prospects:', e.message); }

try {
  const adminHtml = path.join(__dirname, '..', 'public', 'admin.html'); let adminSource = fs.readFileSync(adminHtml, 'utf8');
  ...
  adminSource = adminSource.replace(/<div class="brand"><img src="\/logo-branco\.png" alt="DUIT" style="[^"]*max-width:\d+px;[^"]*"><\/div>/,'<div class="brand"><img src="/logo-branco.png" alt="DUIT" style="display:block;width:100%;max-width:135px;height:auto;object-fit:contain"></div>'); fs.writeFileSync(adminHtml, adminSource, 'utf8');
} catch (e) { console.warn('[admin] não foi possível aplicar o logo branco:', e.message); }

try {
  const adminJs = path.join(__dirname, '..', 'public', 'js', 'admin.js'); let adminJsSource = fs.readFileSync(adminJs, 'utf8');
