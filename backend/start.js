...
try {
  ...
  serverSource = serverSource.replace(/prospects-crm\.js\?v=[^'\"]+/g, 'prospects-crm.js?v=20260910c');
  ...
  // “Todos” não pode parecer “Contactados”: a ordenação por data só fica ativa após clique no cabeçalho.
  source = source.replace("crmDateSort='desc'", "crmDateSort='none'");
  source = source.replace("}).sort((a,b)=>{const av=", "}).sort((a,b)=>{if(crmDateSort==='none')return Number(b.id)-Number(a.id);const av=");
  ...
  if (!source.includes(marker)) source += `\n;(() => { if (document.querySelector('script[data-duit-prospect-actions]')) return; const s=document.createElement('script'); s.src='/js/prospects-actions.js?v=20260910c'; s.dataset.duitProspectActions='1'; document.body.appendChild(s); })();\n`;
  else source=source.replace(/prospects-actions\.js\?v=[^'\"]+/g,'prospects-actions.js?v=20260910c');
  const uiFixMarker='prospects-ui-fix.js';
  if (!source.includes(uiFixMarker)) source += `\n;(() => { if (document.querySelector('script[data-duit-prospect-ui-fix]')) return; const s=document.createElement('script'); s.src='/js/prospects-ui-fix.js?v=20260910c'; s.dataset.duitProspectUiFix='1'; document.body.appendChild(s); })();\n`;
  else source=source.replace(/prospects-ui-fix\.js\?v=[^'\"]+/g,'prospects-ui-fix.js?v=20260910c');
