...
(() => {
  ...
  function decorateTable() {
    document.querySelectorAll('tr[onclick*="openCrmProspect("]').forEach(tr => {
      ...
      const first=tr.querySelector('td'); if(first){let d=first.querySelector('.crm-email-track');if(!d){d=document.createElement('div');d.className='crm-email-track';d.style.marginTop='7px';first.appendChild(d)}d.innerHTML=emailState(p);}
  ...
  function decorateModal(id) {
    ...
    let stateBox=document.getElementById('crm-email-state-box');if(!stateBox){stateBox=document.createElement('div');stateBox.id='crm-email-state-box';stateBox.className='span-2';stateBox.style='padding:10px 12px;background:var(--bg-2);border-radius:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap';form.querySelector('.crm-modal-grid')?.appendChild(stateBox)}stateBox.innerHTML=`<strong style="font-size:12px">Estado do email:</strong> ${emailState(p)}`;
