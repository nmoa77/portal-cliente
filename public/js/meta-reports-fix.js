...
(() => {
  ...
  function numFrom(text, re){const m=(text||'').match(re);return m?Number(String(m[1]).replace(/\./g,'').replace(',','.')):0;}

  function recommendationsFromPage(page){
    const txt=page.textContent||'';
    const views=numFrom(txt,/(\d[\d\.]*)\s+visualiza[cç][oõ]es/i);
    const reach=numFrom(txt,/(\d[\d\.]*)\s+contas/i);
    const interactions=numFrom(txt,/(\d[\d\.]*)\s+intera[cç][oõ]es/i);
    const fbReactions=numFrom(txt,/(\d+)\s+rea[cç][oõ]es/i);
    const fbComments=numFrom(txt,/(\d+)\s+coment[aá]rios?/i);
    const fbShares=numFrom(txt,/(\d+)\s+partilhas?/i);
  ...
  function fixOldConclusion(book){
    const pages=[...book.querySelectorAll('.mr-page')];
    const page=pages.find(p=>/Durante\s+\w+\s+foram\s+publicados/i.test(p.textContent||''));
    if(!page || page.dataset.safeConclusion==='1') return;
    const rec=recommendationsFromPage(page);
    page.dataset.safeConclusion='1';
    page.innerHTML=`<div class="mr-head"><div><div class="mr-brand">Conclusão</div><h2>O que melhorar no próximo mês</h2></div></div><div class="mr-recommendations">${rec.map((r,i)=>`<div class="mr-rec"><div class="mr-rec-n">0${i+1}</div><div><h3>${r[0]}</h3><p>${r[1]}</p></div></div>`).join('')}</div><div class="mr-footer"><span>RELATÓRIO EFETUADO POR @DUIT</span><span>DUIT</span></div>`;
  }

  function ensureStyle(){
    if(document.getElementById('mr-safe-fix-style')) return;
    const s=document.createElement('style');s.id='mr-safe-fix-style';
