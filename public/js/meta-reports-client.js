...
(() => {
  ...
  function previewBlock(){const r=rows.find(x=>Number(x.id)===Number(previewId));if(!r||!r.pdf_path)return'';return `<div class="card" style="margin-bottom:24px;padding:0;overflow:hidden">
    ...
    <div style="background:#e9e9e7;padding:22px;display:flex;justify-content:center;min-height:320px"><img src="/api/meta/reports/${r.id}/preview.png" alt="Pré-visualização do relatório ${monthName(r.ref_month)} ${r.ref_year}" data-report-preview-image style="display:block;width:100%;max-width:1123px;height:auto;box-shadow:0 4px 18px rgba(0,0,0,.14);background:#111" onerror="this.parentElement.innerHTML='<div class=&quot;empty&quot; style=&quot;background:#fff;max-width:620px;width:100%&quot;>A pré-visualização visual ainda não existe para este relatório. Gere novamente o relatório para criar a pré-visualização sem controlos de download.</div>'"></div>
  </div>`;}
